...
impl Plugin for FallingCubesPlugin {
    fn build(&self, app: &mut App) {
        app
            .add_systems(Startup, (setup_scene, spawn_cubes, setup_gravity))
...
fn setup_scene(
    ...
    commands.spawn((
        Name::new("Ground"),
        RigidBody::Fixed,
        Collider::cuboid(ground_collider_size.x, ground_collider_size.y, ground_collider_size.z),
        Transform::from_xyz(0.0, -0.1, 0.0),
        Mesh3d(meshes.add(Mesh::from(Cuboid::new(ground_size.x, ground_size.y, ground_size.z)))),
    ...
    for y in 0..5 {
        for x in 0..5 {
            ...
            commands.spawn ((
                ...
                RigidBody::Dynamic,
                Collider::cuboid(cube_collider_size.x, cube_collider_size.y, cube_collider_size.z),
                Friction::coefficient(0.8),
                Restitution::coefficient(0.4),
                Sleeping::disabled(),
                Transform::from_translation(pos),
                Mesh3d(meshes.add(Mesh::from(Cuboid::new(cube_size.x, cube_size.y, cube_size.z)))),
...
fn setup_gravity(
    mut commands: Commands,
    mut q: Query<&mut RapierConfiguration>) {
...
fn apply_gravity(
    mut q: Query<&mut RapierConfiguration>,
    world_gravity: ResMut<WorldGravity>,
)
{
    ...
    if let Ok(mut cfg) = q.single_mut() {
        ...
        if world_gravity.is_enabled == false {
            *gravity = Vec3::ZERO;
            ...
            info!("Gravity was changed from {:?} to {:?}", *gravity, world_gravity.gravity_force);
